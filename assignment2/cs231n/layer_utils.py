...
def affine_norm_relu_forward(x, w, b,gamma,beta,bn_param):
    a, fc_cache = affine_forward(x, w, b)
    a1, norm_cache = batchnorm_forward(a, gamma, beta, bn_param)
...
def affine_norm_relu_backward(dout, cache):
    ...
    da = relu_backward(dout, relu_cache)
    da1, dgamma, dbeta = batchnorm_backward_alt(da, norm_cache)
    dx, dw, db = affine_backward(da1, fc_cache)
    return dx, dw, db, dgamma, dbeta

def affine_layernorm_relu_forward(x, w, b,gamma,beta,bn_param):
    a, fc_cache = affine_forward(x, w, b)
    a1, norm_cache = layernorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(a1)
    cache = (fc_cache,norm_cache, relu_cache)
    return out, cache
...
def affine_layernorm_relu_backward(dout, cache):
    fc_cache, norm_cache,relu_cache = cache
    da = relu_backward(dout, relu_cache)
    da1, dgamma, dbeta = layernorm_backward(da, norm_cache)
    dx, dw, db = affine_backward(da1, fc_cache)
    return dx, dw, db, dgamma, dbeta
